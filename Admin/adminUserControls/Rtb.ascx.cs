...
public partial class adminUserControls_Rtb : UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        FormList = _entities.Forms.Where(p => p.LanguageId == EnrollAdminContext.Current.DataLanguage.LanguageId).ToList();
        LList = _entities.Lists.Where(p => p.LanguageId == EnrollAdminContext.Current.DataLanguage.LanguageId).ToList();
        VCategoriesList = _entities.VideoCategories.Where(p => p.languageId == EnrollAdminContext.Current.DataLanguage.LanguageId).ToList();
        PCategoriesList = _entities.Def_photoAlbumCategory.Where(p => p.languageId == EnrollAdminContext.Current.DataLanguage.LanguageId).ToList();
        PAlbumsList = _entities.Def_photoAlbum.Where(p => p.languageId == EnrollAdminContext.Current.DataLanguage.LanguageId).ToList();
    }
}