...
namespace eNroll.Admin.adminUserControls
{
    ...
    public partial class MemberSearch : System.Web.UI.UserControl
    {
        ...
        DataTable _memberSearchDataTable = new DataTable();
        ...
        string _searchResultExcellCommandText = string.Empty;
        string _searchResultCountCommandText = string.Empty;
        string _innerJoin = string.Empty;
        int _parametersCount = 0;
        StringBuilder _searchCommandUsersWhere = new StringBuilder();
        StringBuilder _searchCommandUserGeneralWhere = new StringBuilder();
        StringBuilder _searchCommandUserMembershipWhere = new StringBuilder();
        #endregion
        ...
        protected void BtSearchClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ddlBloodType.SelectedIndex > 0)
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.BloodType={0}", ddlBloodType.SelectedItem.Value));
                ...
                if (ddlMaritalStatus.SelectedIndex > 0)
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.MaritalStatus={0}", ddlMaritalStatus.SelectedItem.Value));
                ...
                if (ddlHomeCountry.SelectedIndex > 0)
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.HomeCountry={0}", ddlHomeCountry.SelectedItem.Value));
                ...
                if (ddlWorkCountry.SelectedIndex > 0)
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.WorkCountry={0}", ddlWorkCountry.SelectedItem.Value));
                ...
                if (ddlJobSectors.SelectedIndex > 0)
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.JobSectorNo={0}", ddlJobSectors.SelectedItem.Value));
                ...
                if (!string.IsNullOrEmpty(tbWorkCorparation.Text))
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.WorkCorporation LIKE '%{0}%'", tbWorkCorparation.Text));
                #endregion
                ...
                if (dpLastSchoolGraduateDate.SelectedDate != null && !string.IsNullOrEmpty(dpLastSchoolGraduateDate.SelectedDate.Value.Year.ToString()))
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.LastSchoolGraduateDate >= '{0}'", dpLastSchoolGraduateDate.SelectedDate.Value.Year.ToString()));
                if (dpLastSchoolGraduateDate2.SelectedDate != null && !string.IsNullOrEmpty(dpLastSchoolGraduateDate2.SelectedDate.Value.Year.ToString()))
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.LastSchoolGraduateDate <= '{0}'", dpLastSchoolGraduateDate2.SelectedDate.Value.Year.ToString()));
                #endregion
                ...
                if (dpDeceaseDate.SelectedDate != null && !string.IsNullOrEmpty(dpDeceaseDate.SelectedDate.Value.Year.ToString()))
                {
                    DateTime date = dpDeceaseDate.SelectedDate.Value;
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.DeceaseDate >= '{0}.{1}{2}.{3}{4} 00:00:00.000'",
                        date.Year, date.Month < 10 ? "0" : "", date.Month, date.Day < 10 ? "0" : "", date.Day));
                }

                if (dpDeceaseDate2.SelectedDate != null && !string.IsNullOrEmpty(dpDeceaseDate2.SelectedDate.Value.Year.ToString()))
                {
                    DateTime date = dpDeceaseDate2.SelectedDate.Value;
                    AddWhereParams(DbTable.UserGeneral, string.Format("g.DeceaseDate <= '{0}.{1}{2}.{3}{4} 23:59:59.999'",
                        date.Year, date.Month < 10 ? "0" : "", date.Month, date.Day < 10 ? "0" : "", date.Day));
                }
                ...
                if (!string.IsNullOrEmpty(tbMembershipNumber2.Text))
                    AddWhereParams(DbTable.UserFoundation, string.Format("f.MemberNo <= '{0}'", tbMembershipNumber2.Text));
                ...
                if (dpTerm.SelectedDate != null && !string.IsNullOrEmpty(dpTerm.SelectedDate.Value.Year.ToString()))
                    AddWhereParams(DbTable.UserFoundation, string.Format("f.Term >= '{0}'", dpTerm.SelectedDate.Value.Year.ToString()));
                if (dpTerm2.SelectedDate != null && !string.IsNullOrEmpty(dpTerm2.SelectedDate.Value.Year.ToString()))
                    AddWhereParams(DbTable.UserFoundation, string.Format("f.Term <= '{0}'", dpTerm2.SelectedDate.Value.Year.ToString()));

                if (dpMembershipDate.SelectedDate != null && !string.IsNullOrWhiteSpace(dpMembershipDate.SelectedDate.Value.ToShortDateString()))
                {
                    DateTime date = dpMembershipDate.SelectedDate.Value;
                    AddWhereParams(DbTable.UserFoundation, string.Format("f.MembershipDate >= '{0}.{1}{2}.{3}{4} 00:00:00.000'",
                        date.Year, date.Month < 10 ? "0" : "", date.Month, date.Day < 10 ? "0" : "", date.Day));
                }
                if (dpMembershipDate2.SelectedDate != null && !string.IsNullOrWhiteSpace(dpMembershipDate2.SelectedDate.Value.ToShortDateString()))
                {
                    DateTime date = dpMembershipDate2.SelectedDate.Value;
                    AddWhereParams(DbTable.UserFoundation, string.Format("f.MembershipDate <= '{0}.{1}{2}.{3}{4} 23:59:59.999'",
                        date.Year, date.Month < 10 ? "0" : "", date.Month, date.Day < 10 ? "0" : "", date.Day));
                }

                if (ddlMembershipRelType.SelectedIndex > 0)
                    AddWhereParams(DbTable.UserFoundation, string.Format("f.MemberRelType = {0}", ddlMembershipRelType.SelectedItem.Value));
            ...
            if (btnMemberEdit != null)
            {
                ...
                if (Member != null)
                {
                    ...
                    btnConfirmMemberActive.Visible =
                        EnrollMembershipHelper.UserMembershipState(userId.ToString()) == EnrollMembershipHelper.MembershipType.OnayBekleyen;

                    var btnSetPassive = ((Button)cMemberAddEdit.FindControl("BtnSetPassive"));
                    btnSetPassive.Visible =
                        EnrollMembershipHelper.UserMembershipState(userId.ToString()) == EnrollMembershipHelper.MembershipType.Aktif;

                    var btnSetActive = ((Button)cMemberAddEdit.FindControl("BtnSetActive"));
                    btnSetActive.Visible =
                        EnrollMembershipHelper.UserMembershipState(userId.ToString()) == EnrollMembershipHelper.MembershipType.Pasif;

                    var btnResendActivationCode = ((Button)cMemberAddEdit.FindControl("BtnResendActivationCode"));
                    btnResendActivationCode.Visible =
                        EnrollMembershipHelper.UserMembershipState(userId.ToString()) == EnrollMembershipHelper.MembershipType.Hatali;
        ...
        #region <show> & <download as excel> search result of Members & send Email,Sms
        ...
        protected void BtDownloadUsersExcelList(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(hfSearchResaultExcellCmd.Value))
                {
                    ...
                    var cmdSearchResaultExcell = new SqlCommand { Connection = _oConnection, CommandText = searchCommandExcellText };
                    if (_oConnection.State == ConnectionState.Closed) _oConnection.Open();
                    ...
                    if (_memberSearchExcellDataTable.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow item in _memberSearchExcellDataTable.Rows)
                        {
                            ...
                            var bloodId = ((item["BloodType"] != DBNull.Value && item["BloodType"] != null) ? Convert.ToInt32(item["BloodType"].ToString()) : 0);
                            var bloodType = _entities.BloodTypes.FirstOrDefault(p => p.Id == bloodId);

                            var admin = ((item["isAdmin"] != DBNull.Value && item["isAdmin"] != null) && Convert.ToBoolean(item["isAdmin"]));
                            var gender = ((item["Gender"] != DBNull.Value && item["Gender"] != null) ? Convert.ToInt32(item["Gender"].ToString()) : 0);
                            var decease = ((item["Decease"] != DBNull.Value && item["Decease"] != null) && Convert.ToBoolean(item["Decease"]));
                            var memberState = Convert.ToInt32(item["MemberState"]);
                            var termLeader = ((item["TermLeader"] != DBNull.Value && item["TermLeader"] != null) && Convert.ToBoolean(item["TermLeader"]));
                            var vip = ((item["Vip"] != DBNull.Value && item["Vip"] != null) && Convert.ToBoolean(item["Vip"]));
                            ...
                            if (item["MaritalStatus"] != DBNull.Value && item["MaritalStatus"] != null)
                            {
                                ...
                                if (maritalStatu == 1) maritalStatus = AdminResource.lbMarried; else if (maritalStatu == 2) maritalStatus = AdminResource.lbSingle;
                            }

                            var homeCountry = ((item["HomeCountry"] != DBNull.Value && item["HomeCountry"] != null) ?
                                Convert.ToInt32(item["HomeCountry"].ToString()) : 0);
                            var homeCity = ((item["HomeCity"] != DBNull.Value && item["HomeCity"] != null) ?
                                Convert.ToInt32(item["HomeCity"].ToString()) : 0);
                            var homeTown = ((item["HomeTown"] != DBNull.Value && item["HomeTown"] != null) ?
                                Convert.ToInt32(item["HomeTown"].ToString()) : 0);
                            var workCountry = ((item["WorkCountry"] != DBNull.Value && !string.IsNullOrWhiteSpace(item["WorkCountry"].ToString())) ?
                                Convert.ToInt32(item["WorkCountry"]) : 0);
                            var workCity = ((item["WorkCity"] != DBNull.Value && !string.IsNullOrWhiteSpace(item["WorkCity"].ToString())) ?
                                Convert.ToInt32(item["WorkCity"].ToString()) : 0);
                            var workTown = ((item["WorkCity"] != DBNull.Value && !string.IsNullOrWhiteSpace(item["WorkTown"].ToString())) ?
                                Convert.ToInt32(item["WorkTown"].ToString()) : 0);
                            var jobNo = ((item["JobNo"] != DBNull.Value && item["JobNo"] != null) ?
                                Convert.ToInt32(item["JobNo"].ToString()) : 0);
                            var jobSectorNo = ((item["JobSectorNo"] != DBNull.Value && item["JobSectorNo"] != null) ?
                                Convert.ToInt32(item["JobSectorNo"]) : 0);
                            var memberRelType = ((item["MemberRelType"] != DBNull.Value && item["MemberRelType"] != null) ?
                                Convert.ToInt32(item["MemberRelType"]) : 0);
                            ...
                            newRow[AdminResource.lbGender] = (gender == 1 ? AdminResource.lbMan : (gender == 2 ? AdminResource.lbWoman : ""));
                            newRow[AdminResource.lbMemberState] = (memberState == 1 ? AdminResource.lbActive :
                                (memberState == 0 ? AdminResource.lbPassive : "Engellenmiş"));
                            newRow[AdminResource.lbTermLeader] = (termLeader ? AdminResource.lbYes : AdminResource.lbNo);
                            newRow[AdminResource.lbState] = (Convert.ToBoolean(item["uState"]) ? AdminResource.lbActive : AdminResource.lbPassive);
                            ...
                            newRow[AdminResource.lbMemberRelType] = (memberRelType != 0 ? _entities.FoundationRelType.First(p => p.Id == memberRelType).Name : "");
                            newRow[AdminResource.lbHomeCountry] = (homeCountry != 0 ? _entities.Countries.First(p => p.Id == homeCountry).Name : "");
                            newRow[AdminResource.lbWorkCountry] = (workCountry != 0 ? _entities.Countries.First(p => p.Id == workCountry).Name : "");
                            newRow[AdminResource.lbHomeCity] = (homeCity != 0 ? _entities.Cities.First(p => p.Id == homeCity).Name : "");
                            newRow[AdminResource.lbWorkCity] = (workCity != 0 ? _entities.Cities.First(p => p.Id == workCity).Name : "");
                            newRow[AdminResource.lbHomeTown] = (homeTown != 0 ? _entities.Towns.First(p => p.Id == homeTown).Name : "");
                            newRow[AdminResource.lbWorkTown] = (workTown != 0 ? _entities.Towns.First(p => p.Id == workTown).Name : "");
                            newRow[AdminResource.lbJobSector] = (jobSectorNo != 0 ? _entities.JobSectors.First(p => p.Id == jobSectorNo).Name : "");
                            newRow[AdminResource.lbJob] = (jobNo != 0 ? _entities.Jobs.First(p => p.Id == jobNo).Name : "");

                            newRow[AdminResource.lbBirthDate] = (!string.IsNullOrEmpty(item["BirthDate"].ToString()) ?
                                                                Convert.ToDateTime(item["BirthDate"]).ToShortDateString() : "");
                            newRow[AdminResource.lbMembershipDate] = (!string.IsNullOrEmpty(item["MembershipDate"].ToString()) ?
                                                                Convert.ToDateTime(item["MembershipDate"]).ToShortDateString() : "");
                            newRow[AdminResource.lbMarriageDate] = (!string.IsNullOrEmpty(item["MarriageDate"].ToString()) ?
                            ...
                            newRow[AdminResource.lbDeceaseDate] = (!string.IsNullOrEmpty(item["DeceaseDate"].ToString()) ?
                                                                Convert.ToDateTime(item["DeceaseDate"]).ToShortDateString() : "");

                            newRow[AdminResource.lbLastSchoolGraduateDate] = ((item["LastSchoolGraduateDate"] != DBNull.Value && item["LastSchoolGraduateDate"] != null) ?
                                Convert.ToDateTime(item["LastSchoolGraduateDate"]).Year.ToString() : "");
                            newRow["TC"] = ((item["TC"] != DBNull.Value && item["TC"] != null) ? item["TC"].ToString() : "");
