...
public partial class M_UserControls_Products : UserControl
{
    ...
    private void AltKategorileriListele(int catId)
    {
        ...
        stringBuilder.AppendLine("<ul data-role='listview' data-inset='true' data-corners='false'>");
        foreach (var categories in subCategories)
        {
            stringBuilder.AppendFormat("<li><a href='urunler-{0}-1'>{1}</a>" +
            ...
            foreach (Control c in control.Controls)
            {
                if (c is HyperLink)
                {
                    ...
                    if ((!string.IsNullOrEmpty(Request.Url.AbsolutePath)) && (!string.IsNullOrEmpty(Request.Url.Query)))
                    {
                        ...
                        {
                            currentLink.NavigateUrl = currentLink.NavigateUrl.Replace("/m/Products.aspx?",
                                                                                      "/m/urunler-");
                        }
