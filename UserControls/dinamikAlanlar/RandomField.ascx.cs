...
public partial class UserControls_RandomField : UserControl
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            var customerRandom =
                oEntities.Customer_Random.Where(p => p.LanguageId == oGeneralInfo.languageId && p.State).ToList();
            ...
            var detailLink = string.Format("<a class='button' href='sayfa-r-{0}-{1}'>{2}</a>",
                                           selectedItem.Id, UrlMapping.cevir(selectedItem.Title), Resource.details);
            ltRandom.Text = string.Format("{0}{1}", selectedItem.Summary, detailLink);
        }
