...
public partial class M_UserControls_duyuru_NoticeList : UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            HiddenField2.Value = DateTime.Now.ToShortDateString();
            ...
            foreach (Control c in control.Controls)
            {
                if (c is HyperLink)
                {
                    ...
                    if ((!string.IsNullOrEmpty(Request.Url.AbsolutePath)) && (!string.IsNullOrEmpty(Request.Url.Query)))
                    {
                        ...
                        {
                            currentLink.NavigateUrl = currentLink.NavigateUrl.Replace("/m/Notices.aspx?",
                                                                                      "/m/duyurular-");
                        }
    ...
    protected void HyperLink1_DataBinding(object sender, EventArgs e)
    {
        ...
        Notices duyuru = ent.Notices.Where(p => p.noticeId == noticeId).First();
