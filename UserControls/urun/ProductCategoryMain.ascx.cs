...
public partial class UserControls_ProductCategoryMain : UserControl
{
        ...
        foreach (Control control in DataPager1.Controls)
        {
            foreach (Control c in control.Controls)
            {
                if (c is HyperLink)
                {
                    ...
                    {
                        if (Request.Url.AbsolutePath != "/ProductCategoryMain.aspx")
                        {
                            if (Request.Url.PathAndQuery.IndexOf("&") != -1)
                            {
                                currentLink.NavigateUrl = currentLink.NavigateUrl.Replace(
                                    Request.Url.PathAndQuery + "&", "../../urunkategorileri-");
                            }
                            else
                            {
                        ...
                        {
                            currentLink.NavigateUrl = currentLink.NavigateUrl.Replace("/ProductCategoryMain.aspx?",
                                                                                      "../../urunkategorileri-");
                        }
