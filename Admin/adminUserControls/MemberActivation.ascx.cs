...
{
    public partial class MemberActivation : System.Web.UI.UserControl
    {
        ...
        const string SqlSelect = "Select *,u.Id as uId From Users as u " +
        ...
        EntityDataSource edsMember = new EntityDataSource();
            ...
            if (btnMemberEdit != null)
            {
                try
                {
                    ...
                    if (Member != null)
                    {
                        ...
                        BtnDeleteAll.Visible = EnrollMembershipHelper.UserMembershipState(userId.ToString()) == EnrollMembershipHelper.MembershipType.Hatali;
        ...
        protected void gVMembers_OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (imgBtnEdit != null)
            {
                ...
                if (HttpContext.Current.User.Identity.IsAuthenticated &&
                    !EnrollMembershipHelper.IsAuthForThisProcess(userId, EnrollMembershipHelper.GetUserIdFromEmail(HttpContext.Current.User.Identity.Name)))
                {
                    imgBtnEdit.OnClientClick = string.Format("javascript:showWarningToast('{0}');return false;", AdminResource.msgNoAuth);
                }
            }
            if (imgBtnDelete != null)
            {
                ...
                {
                    imgBtnDelete.OnClientClick = string.Format("javascript:showWarningToast('{0}');return false;", AdminResource.msgNoAuth);
                }
                else
                {
            ...
            if (imgBtnSetActive != null)
            {
                ...
                if (!EnrollMembershipHelper.IsAuthForThisProcess(userId,EnrollMembershipHelper.GetUserIdFromEmail(HttpContext.Current.User.Identity.Name)))
                {
                    imgBtnSetActive.OnClientClick = string.Format("javascript:showWarningToast('{0}');return false;", AdminResource.msgNoAuth);
                }
                else
                {
            ...
            if (imgBtnSetPassive != null)
            {
                ...
                if (!EnrollMembershipHelper.IsAuthForThisProcess(userId,EnrollMembershipHelper.GetUserIdFromEmail(HttpContext.Current.User.Identity.Name)))
                {
                    imgBtnSetPassive.OnClientClick = string.Format("javascript:showWarningToast('{0}');return false;", AdminResource.msgNoAuth);
                }
                else
                {
            ...
            if (btnSetActive != null)
            {
                ...
                if (!EnrollMembershipHelper.IsAuthForThisProcess(userId, EnrollMembershipHelper.GetUserIdFromEmail(HttpContext.Current.User.Identity.Name)))
                {
                    btnSetActive.OnClientClick = string.Format("javascript:showWarningToast('{0}');return false;", AdminResource.msgNoAuth);
                }
                else
                {
        ...
        public string GetMembershipType(string userIdStr)
        {
            ...
            if (!string.IsNullOrEmpty(userIdStr))
            {
                ...
                if (userId > 0)
                {
                    ...
                    {
                        var foundationRelType = _entities.FoundationRelType.FirstOrDefault(p => p.Id == userFoundation.MemberRelType);
        ...
        protected void ddlMemberStatesFilter_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(selectedFilter))
            {
                ...
                if (data.Length > 2)
                {
                    ...
                    hfMemberEmailActivationFilter.Value = (!string.IsNullOrWhiteSpace(data[2]) ? Crypto.Encrypt(data[2]) : "");
                }
            }
            ...
            try
            {
                try
                {
                    ...
                    if (_memberSearchDataTable.Rows.Count > 0)
                    {
                        ...
                        if (mailCount > 0)
                        {
                            MessageBox.Show(MessageType.Notice, string.Format(AdminResource.msgVerificationCodeSentMember, mailCount.ToString()));
                        }
                    }
                    else
                    {
